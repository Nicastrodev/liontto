...
namespace LionttoMoveis.Repository
{
    ...
    public class MaterialRepository : MySqlRepository<Material>
    {
        ...
        public async Task<(int Produtos, int Movimentacoes)> ObterResumoUsoAsync(int id)
        {
            var produtos = await _ctx.MateriaisDoProduto
                .Where(m => m.MaterialId == id)
                .Select(m => m.ProdutoId)
                .Distinct()
                .CountAsync();

            var movimentacoes = await _ctx.Movimentacoes
                .CountAsync(m => m.MaterialId == id);

            return (produtos, movimentacoes);
        }

        public async Task<bool> ExcluirComVinculosAsync(int id)
        {
            var material = await _ctx.Materiais.FindAsync(id);
            if (material is null)
                return false;

            await _ctx.MateriaisDoProduto
                .Where(m => m.MaterialId == id)
                .ExecuteDeleteAsync();
